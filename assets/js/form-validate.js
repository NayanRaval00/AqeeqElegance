	...
	//}else if( f.phone.value == '' ){
		...
		notice( f.message );
		 ...
		 jQuery.ajax({
			url: 'mail.php',
			type: 'post',
			data: jQuery('form#AqeeqElegance-contact').serialize(),
			complete: function(data) {
				jQuery('#form_status').html(data.responseText);
				jQuery('#AqeeqElegance-contact').find('input,textarea').attr({value:''});
				jQuery('#AqeeqElegance-contact').css({opacity:1});
				jQuery('#AqeeqElegance-contact').remove();
			}
		});
		jQuery('#form_status').html('<span class="loading">Sending your message...</span>');
		jQuery('#AqeeqElegance-contact').animate({opacity:0.3});
		jQuery('#AqeeqElegance-contact').find('input,textarea,button').css('border','none').attr({'disabled':''});
	}
...
function notice( f ){
	jQuery('#AqeeqElegance-contact').find('input,textarea').css('border','none');
